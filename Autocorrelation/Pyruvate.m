...
standardsigma = [0.0056855360   0.002689758   0.08209500;
   ...
   mu0 = 4E-7 * pi;
   time = (1:(MAXDEL + 1)) * dt;
   moleculerange = 1:9;                     % nuclei at indices 1-9 are pyruvate
   waterrange = 10:(size(traj, 2)-1);        % nuclei at range 10-(end-1) are water
   counterionrange = size(traj, 2);         % last nucleus is sodium
   gamma(moleculerange) = [muC 0 0 muC 0 muC muH muH muH];
